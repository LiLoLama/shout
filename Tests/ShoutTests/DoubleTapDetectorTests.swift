...
final class DoubleTapDetectorTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        detector = DoubleTapDetector()   // Fenster 0.7 s, Sperrfrist 0.25 s
    }
    ...
    func testZweiSchnelleTippsStarten() {
        XCTAssertEqual(detector.handleDown(at: 10.0, isRecording: false), .armed,
                       "der erste Tipp startet nichts, macht aber scharf")
    ...
    /// (Auf der Grenze selbst wird nicht geprüft — 10.7 - 10.0 ist in Double
    /// minimal größer als 0.7, das wäre ein Test über Fließkomma-Rundung.)
    func testFensterrand() {
        _ = detector.handleDown(at: 10.0, isRecording: false)
        XCTAssertEqual(detector.handleDown(at: 10.69, isRecording: false), .start)

        var zweiter = DoubleTapDetector()
        _ = zweiter.handleDown(at: 10.0, isRecording: false)
        XCTAssertEqual(zweiter.handleDown(at: 10.71, isRecording: false), .armed,
                       "zu spät ist kein Doppeltipp, sondern ein neuer erster Tipp")
    }

    /// Zu langsam getippt → keine Aufnahme. Genau das schützt vor Versehen.
    func testZweiLangsameTippsStartenNicht() {
        XCTAssertEqual(detector.handleDown(at: 10.0, isRecording: false), .armed)
        XCTAssertEqual(detector.handleDown(at: 11.0, isRecording: false), .armed)
    }
    ...
    func testDritterTippInDerSperrfristStopptNicht() {
        _ = detector.handleDown(at: 10.0, isRecording: false)
        XCTAssertEqual(detector.handleDown(at: 10.2, isRecording: false), .start)
        XCTAssertEqual(detector.handleDown(at: 10.35, isRecording: true), .ignored)
    }
    ...
    func testNachDemStoppBrauchtEsWiederZweiTipps() {
        ...
        XCTAssertEqual(detector.handleDown(at: 12.1, isRecording: false), .armed,
